...
| Real local stack | Three Chromium flows passed using the actual auth-service, PostgreSQL, frontend, and development CLI: fresh password login, native TOTP MFA login, and denial. The suite verifies matching codes, discovery, profiles, private credentials, rotation, session logout, and continued browser login. Videos and transcripts are generated by `scripts/test-local-demo.py`. |
